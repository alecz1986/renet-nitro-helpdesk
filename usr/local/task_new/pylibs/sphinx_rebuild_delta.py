...
dbconn = Session()
f = open('/usr/local/task_new/pylibs/last_id_sphinx2019.txt', 'r')
try:
    last_id = f.readlines()[0]
except:
    last_id = None
f.close()
...
if last_id is not None:
    ths = dbconn.query(Threads, Tasktype).filter(Threads.title_id == Tasktype.id).filter(Threads.id > int(last_id))
else:
    ths = dbconn.query(Threads, Tasktype).filter(Threads.title_id == Tasktype.id).order_by(Threads.id.desc()).limit(20)
dbconn.close()
