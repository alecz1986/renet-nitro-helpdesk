...
ths = dbconn.query(Threads, Tasktype).filter(Threads.title_id == Tasktype.id).filter(Threads.creation_date.like('2019%'))#.limit(20).offset(1000)
...
f = open('/usr/local/task_new/pylibs/last_id_sphinx2019.txt', 'w')
f.write(str(t.id))
f.close()
