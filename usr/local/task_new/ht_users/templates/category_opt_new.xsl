<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xi="urn:xi" xmlns:css="urn:css">
  <xsl:template match="/">
    ...
    <xsl:for-each select="/R/data/categorys/category">
      <xsl:text> / </xsl:text>
      <xsl:element name='a'>
        <xsl:attribute name='href'>/category?category_id=<xsl:value-of select="@id"/></xsl:attribute>
    ...
    <ul class='menu' width='500px'>
      ...
      <xsl:for-each select="/R/data/categorys_link/category">
        <li>
          <xsl:element name='a'>
            <xsl:attribute name='href'>/category?category_id=<xsl:value-of select="@link"/></xsl:attribute>
            <xsl:value-of select="@name"/>
          </xsl:element>
          <br/>
        </li>
