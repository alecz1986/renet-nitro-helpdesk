      ...
      <table class="tsimple">
        ...
        <xsl:choose>
          <xsl:when test="/R/form/action/value[text()='new']">
            <thead>
              <th colspan="2"><div id="st._head_" css:color="green">Создание новой записи</div></th>
            </thead>
          </xsl:when>
          <xsl:when test="/R/form/action/value[text()='edit']">
            <thead>
              <th colspan="2"><div id="st._head_" css:color="red">Режим редактирования</div></th>
            </thead>
          </xsl:when>
        </xsl:choose>
      </tr>
        <tbody>
          ...
          <xsl:for-each select="/R/form/*">
            ...
            <td colspan="2"><div>
                    <input type="submit" id="se.submit" value="  задать фильтр  "/>
            </div></td>
          </tr>
        </tbody>
      </table>
