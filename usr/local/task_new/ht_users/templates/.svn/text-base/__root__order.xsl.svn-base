  ...
  <body onLoad='start()'>
    ...
    <div id='site-center'>
      ...
      <div class='left-column' css:padding-top="5px">
        ...
        <br/>
        <div>
          <noindex> <script type='text/javascript'><xsl:comment><![CDATA[
                document.MAX_ct0 ='';
    ...
    <xsl:if test="($k_iter)/*">
      <ul class="menu">
       <xsl:for-each select="($k_iter)/*">
        <xsl:choose>
          <xsl:when test="/R/data/@category_id='321374' or /R/data/@category_id='280555'">
         ...
         <xsl:otherwise>
          <xsl:if test="@desk='yes' and @id!=321445">
            <li>
              <xsl:element name='a'>
                <xsl:attribute name="css:white-space"><xsl:text>pre-wrap</xsl:text></xsl:attribute>
                <xsl:attribute name='href'>/<xsl:value-of select='@template'/>?category_id=<xsl:value-of select='@id'/></xsl:attribute>
